...
# 企业业务处理的四层架构（SDLC 等价映射）

> 7 轮探索的 capstone synthesis。业务处理从"人操作工具"变成"Agent 在分层平台上自主加工信息"——这个变化在每一层同时发生。
...
1. **BPM 和 SDLC 共享同一个四层架构。** 不管加工的是代码还是合同，四层架构（前端/中端/后端/治理）是通用的。每一层都在经历同样的范式转移。
