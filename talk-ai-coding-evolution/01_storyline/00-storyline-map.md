...
> 状态：**v1.6**。已完成 V7 的 P1-P23 叙事、文字与视觉复核。五层是注意力与工程对象的叠加，不是运行时大小或严格包容关系；落点 = harness + DSH；立场是「先借现成，边界自己定」。
...
整场只沿三问推进：信息怎样形成行动；行动怎样可靠并被 Loop / Graph 组织；当运行边界进入业务，哪些可借、哪些必须自己决定。DSH 不是独立插入的话题，而是最后一问的实例化回答。
...
- **因此 Harness 必须在手**：不是都自己做，而是关键时刻 owner 有 final say。DSH 用 registry、adapter + capability seam、enforced gate 与 append-only session log 使四项决定可操作。
- 回扣：灵活 harness 不会替你设计 loop 或 graph；它让这些组织能力能在清楚的合同、门禁与运行记录上组合起来。

### 收尾
- 回答：一人公司要掌握到 **harness 这一层**。掌握深度不按工具数量算，而按对运行边界的责任算：会用成熟 Harness、看懂运行边界、需要时握住 final say。
- 带走一句：**部件可借，边界自己定。**
