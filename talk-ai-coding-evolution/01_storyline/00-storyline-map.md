...
> 状态：**v1.3**。已校准五层的关系：它们是注意力与工程对象的叠加，不是运行时大小或严格包容关系。落点 = harness + DSH；立场是「从现成开始，但保有改变边界的能力」。
...
**harness**：先借现成的可靠运行环境；一旦任务要求改模型、工具、执行世界、门禁或知识组织，就必须能改变它的边界。
...
### 第四幕：DSH（落点）
- DSH = 一个**灵活**的 harness：它把模型、工具、能力后端、规则与默认 loop 做成可组合的插件树，
  因而给你的是能改 harness 本身的框架。
