...
- 已验证生成页：P1；P49 作为与 P1 配对但不重复的收尾画面。其他页面按叙事任务逐页判断，不预先限定总量。
- 可选 A/B：P8、P20。Image 2 可以承担图文一体的视觉主体，但容量分叉、三层 harness 关系和关键标签必须保留可编辑覆盖层。
...
- 允许生成与场景不可分割的次要标签、界面字样或环境文字，但必须逐字核对，且不能承担观众必须准确读到的结论。
...
- **可编辑层**：标题、副标题、kicker、callout、关键结论、数字、引用、工件、门禁、责任签名、连接关系分别由谁渲染？
...
5. 检查资产：没有未经核对或承担关键结论的生成文字；无品牌、无 watermark、无低清边缘和异常材质。
