# 50 页视觉分镜（v0.8 · 历史设计依据）

> 本文件记录进入完整生产前的布局族、视觉 token 和早期逐页假设。v0.13–v0.14 已改变多页主体实现；当前页面清单、QA 与 REVIEW 入口以 `../CURRENT.md` 为准。
...
以下是当时进入完整生产前的四页验证方案。该样片门已于 v0.11 通过，保留本节用于解释视觉语言的形成过程：
...
样片通过后的生产工作已经完成并迭代到 v0.14。生产事实稿仍必须适配既定布局族，不以缩小字号或堆卡片补救；当前文案见 `../04_drafts/ppt-text-v0.14.md`。
