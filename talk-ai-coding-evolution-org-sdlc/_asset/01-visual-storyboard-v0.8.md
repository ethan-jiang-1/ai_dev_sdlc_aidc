...
允许生成图包含经过核对、与场景不可分割的次要文字；标题、副标题、kicker、callout、关键结论和精确关系始终由 PowerPoint / Graphviz 控制。若生成资产不能同时满足三秒可读、术语准确和文字安全区，就回到可编辑结构。
...
| 页 | 布局 | 资产 | 主画面 | 视觉推进 |
|---|---|---|---|---|
| P1 | L1 | IMG + PPT | 一座纸模软件价值流：线性轨道在构建处高速压缩，路径在远端开始弯成受控闭环；标题留在负空间 | 先给“整条系统”而不是 AI 人物 |
...
| P8 | 容量失配 | 当前用 PowerPoint 可编辑结构；Image 2 可继续作图文主体 A/B | 3 秒内看懂“宽产出、窄控制、排队或欠审” |
| P20 | 核心视觉语法 | 当前用 PowerPoint 可编辑 harness 剖面；Image 2 可提供纸模主体，三层关系用可编辑覆盖层 | 圈住/拦住/看清无需长解释；后续测试、发布能复用 |
...
5. **可编辑门**：流程、门禁、责任人、反馈、graph、log 都是 PowerPoint/Graphviz 可编辑或可重建结构；位图只承载经过核对的次要视觉文字，不承载关键标签与结论。
6. **分层门**：每页能说清 Image 2 承担哪层表达、PowerPoint 承担哪些关键文字、文字安全区在哪里；图像和文字必须共同完成一个判断。
7. **来源门**：外部截图、外部视觉和非平凡主张都能进入 speaker notes 的 `[Sources]`；仿真工件明确标作 illustrative。
8. **密度门**：样片使用 36 pt 以上标题、16 pt 以上正文仍无需拥挤或超出安全区。
