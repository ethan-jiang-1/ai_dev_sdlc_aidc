...
- [x] **ImageGen 边界**：P1/P49 可用定制位图；P8/P20 仅做 A/B；P42 与复杂拓扑/日志保持可编辑。prompt 见 `_asset/02-imagegen-prompts-v0.8.md`。
...
- [ ] 将 50 页内容压成 audience-facing 生产事实稿；逐页通过朗读检查，再控制上屏密度与 speaker notes 分工。
...
- [ ] 当前下一步：用 GPT Image 2 生成 P1/P49 概念场景候选，并与可编辑结构合成样片 → 用户 REVIEW 视觉方向 → 写生产事实稿（`04_drafts/`）→ 母版/50 页 PPTX（`05_output/`）。
