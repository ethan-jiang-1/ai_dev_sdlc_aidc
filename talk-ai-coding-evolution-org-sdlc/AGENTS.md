...
6. **逐页选择主体实现**：按 `CONTEXT.md` 在原生主体、生成主体和混合主体中选择。Image 2 的采用只看表达收益；当原生图元难以自然表达复杂定性关系时，主动使用生成式图文主体，不按封面或普通页身份分配图片。
7. **页面文案保持统一**：主标题、副标题、眉题、强调句、页码和固定术语由 PowerPoint 文字层渲染；生成资产必须为这些页面文案预留明确安全区。
8. **主体内嵌文字允许承担局部解释**：Image 2 可以把局部说明、标签、界面字样与场景合成为丰富的图文主体，不把它降格为无字插画工具。所有主体内嵌文字逐字核对；有错字、歧义、术语漂移或演示距离下不可读时，重新生成或改由文字层承载。
9. **精确关系保持可编辑**：工件顺序、门禁位置、责任归属、因果方向、数字、引用、graph、log 和关键论证使用 PowerPoint / Graphviz 可编辑结构。Image 2 主要强化拥塞、速度、压迫、闭环、边界和材质等感知关系。
10. **逐页完成标准**：能回答「为什么采用这种页面主体、Image 2 的表达收益是什么、页面文案与主体内嵌文字怎样分工、哪些关系必须保持精确、文字安全区在哪里」；答不清时不进入渲染。
