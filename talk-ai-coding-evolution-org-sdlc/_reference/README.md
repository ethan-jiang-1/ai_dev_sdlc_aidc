...
| symlink | 指向 | 装了什么 | 在软件 SDLC talk 里干什么 |
|---|---|---|---|
...
| `rawdata_dsh-digested/` | `deepseek-harness/_digested` | DSH 源码**消化分析** | 第三幕「共享控制面」的底层证据（③） |
...
后三份专题源只补生态、收益与成熟度证据。Anthropic Playbook 是**软件 SDLC 视角的官方对照物**：同讲「闭环 + 六阶段 + 人在门禁」，可作为本 talk 论点的外部锚点（注意：它是 Claude 生态实现，引用时类比而非照搬）。
