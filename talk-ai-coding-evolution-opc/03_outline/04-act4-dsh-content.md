# 第四幕：DSH 的定位与 Harness 最终控制权（v1.7）
...
**上屏**：模型 / 工具 / 执行后端 / 规则与 Loop；共同合同 + 共同事实（append-only session log）。

**定义**：DSH 是一套可组合的 Harness runtime：模型、工具、能力后端、规则与 loop 围绕共同合同装配、替换；每一次发生的行动仍进入共同事实，即 append-only session log。

**讲点**：P18 的三项收益来自共同合同和共同事实，不来自插件数量。默认 loop 只是其中一个驱动插件，不是整个系统；插件扩展 Harness，Harness 负责运行合同与事实记录。
...
## P21 Harness 在手，不是全部自建，而是 owner 有 final say（1.5 min）
...
**机制证明**：registry 管接入；adapter + capability seam 管替换；enforced gate 管放行；append-only session log 支持重建事实。机制是证据，不在本页重新堆机制名。

**讲点**：这不是“所有东西都自己写”。模型、工具、后端、插件都可以借；“在手”指 owner 对能力能否进入、实现能否替换、门禁是否放行、事实能否重建拥有最后决定权。**部件可以借；最终决定权必须留给 owner。**

## P22 OPC 要掌握到 Harness 这一层（2 min）
...
**讲点**：不是为了造框架，而是知道何时借、何时改、何时必须自己决定。掌握深度不按工具数量算，而按对运行边界的责任算。
