...
| 版 | 思路 | 问题（用户反馈） |
|---|---|---|
| v0.1 | harness 单焦点：「组织唯一该集中建设的是 harness」+ 23 页 | 被 playbook 重定调：对象扩为「研发整体组织，从产品到实施」 |
...
**v0.5 为什么对**：SDLC 六阶段是「组织从产品到实施」最自然的轴（宽度）；五层是每个阶段「本质是什么」的点破（深度）；DSH 是「把这条原则做成运行时」的收束。三者各司其职，谁也不吞谁——**五层作引子点破本质，不强扭成独立章节**。

## 四、关键决策与理由

1. **对象 = 研发整体组织**（CTO/平台/产品/QA/安全/发布/运维），不是 OPC、不是混合听众——每页落「组织该建什么/谁负责/怎么治理/怎么度量」。
2. **A 档单场加长**（75–90 min，~50 页）——内容量决定，不再受 23 页约束。
3. **独立成篇**——不依赖、不承接 `deck_ai_sdlc_keynote`（听众不同：那边是战略隐喻，这边是操作级转型）。
4. **题目**：AI-Native 研发组织：从产品到实施的 SDLC 转型之道；**slogan**：代码不再是瓶颈，流程才是。
5. **harness 开场先立住（P5）**，Build/Test/Deploy 再展开——它是临界点，不能晚出。
6. **DSH 三条腿加厚第三幕**（P41–P47）——让 DSH 从「可组合 runtime 一句带过」变成「怎么帮流程」的实料，并回指第二幕。
7. **工具不可知命名**：`CLAUDE.md` → `AGENTS.md`、`.claude/skills` → skills 目录、`.claude/agents` → subagents——随大流，不绑定 Claude Code。

## 五、统一命题（一句话焊点）

> 代码不再是瓶颈。组织真正缺的，不是更强的模型，而是**把「怎么正确参与」外置成系统**——工件链（要什么/为什么/怎么做）、gate（对错怎么判）、归属（改哪里/谁负责）。沿六阶段落地，DSH 把它做成可组合 runtime。

这一句把「六阶段（在哪发生）× 五层（有多深）× DSH（怎么做成运行时）」焊成一条线。三条腿（知识外置=工件链、正确路径=归属、可执行反馈=gate）正是它。

## 六、当前状态

- 50 页页面职责已铺完（`../03_outline/01`–`03`），第三幕 DSH 已加厚。
- 待：用户 review 页面职责 → 校准页数/时间 → 写 `04_drafts/` 生产事实稿 → 选模板（`_asset/`）→ 产 PPTX（`05_output/`）。
