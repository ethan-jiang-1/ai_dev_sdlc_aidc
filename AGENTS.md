...
1. 根目录 `README.md` —— 全景：三层结构（研究/实践/交付）、仓库外素材、当前哪一场在跑。
...
- `03_practice/` 是 2026-09-21 自研究层拆出的**实践层**（SDLC 工程实践与方法论，用户判定其内容已是 practice 而非 research），含三个主题：`requirements_engineering/`、`spec_driven_development/`、`beyond_spec_driven_development/`。三个主题互为兄弟、互相有相对指针，动手前先读各自 README 的分工约定。
