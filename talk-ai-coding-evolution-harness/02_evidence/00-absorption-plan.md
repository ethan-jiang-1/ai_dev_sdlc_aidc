...
> 依据 `../_reference/README.md` 列出的九份上游素材。本文件定"进什么货"和"什么话不能说"。
...
| P32 | Everything is a Plugin（Cordis）；**无特权核心**；"There is no privileged core to patch" | 同上 §5.5 发现 1；`rawdata_dsh-digested/system/00-map.md` |
| P33 | 四种运行模式 = 四套默认插件集（标准 / PTC / 极简 / 创造） | 同上 §5.5 发现 2 |
| P34 | Goal / Ralph / workflow / Spawn-Fork；层级式而非 swarm | 同上 §5.5 发现 5 |
| P35 | append-only 类型化 SessionEvent 日志（唯一真源）；模型所见即被记录；回放/恢复/分叉/审计是投影 | 同上 §5.5 发现 4 |
| P36 | 官方插件市场未上线；第三方商店口径不可比；v0.1 接口快变；二次开发量高于 Product First | 同上 §5.5 发现 7/8 + §5.5 合规小节 |
| P37 | **Pi 的结构好在哪**：三层同心接缝（operations / tool / extension）按"变化轴"裁剪接口大小；统一注入点；分发闭环；生命周期护栏（两阶段绑定、stale 保护、**fail-close**）。**代价在哪**：四条短板——① **无默认沙箱**（扩展 = 宿主进程同等权限，`pi install` 陌生包 ≈ 执行任意代码）② 无 MCP/ACP（生态锁定，外部工具付适配层税）③ 内层 AgentLane 半成品 ④ 无扩展 API 版本契约。补充：~9 万 star（趋势值） | `rawdata_pi-digested/harness/01-Architecture/`（1.1–1.4）、`.../harness/02-Boundaries/`（2.1–2.4）；对照 `rawdata_harness-selection-final/final_v3.md` §5.4 |
| P38 | 两条路线的共同点；**"对陌生插件安全：两个都烂"**（Pi 无沙箱；DSH 插件化 ≠ 安全） | `rawdata_pi-faq-on-digested/06_pi_vs_dsh/04_strengths_weaknesses.md` 逐面强弱表；`.../03_mechanism_table.md` |
| P38 | 本 talk 的落点（两条路线的共同点） | 立场（A8） |
...
1. **瓶颈已从生成移到交付**（A1）。
2. **Loop / Graph 补不上缺失的执行边界，所以问题落回 harness**（A3）。
3. **同一个模型换 harness 就是两套交付系统**（A6）——证据偏弱，必须标注。
4. **团队里那圈东西最容易散，一致性是团队专属问题**（A7）——本 talk 的观察，无外部素材。

## 三、口径红线（引用前必查）

| # | 事项 | 正确口径 |
|---|---|---|
| 1 | Codex 的能力提升数字 | ⚠️ **厂商一手自述**（developers.openai.com 博客），无第三方审计；ARC-AGI-3 13.3% → 38.3%；**一手表述是"输出 token 少 6 倍"**，不要说成"同任务 token 降 1/6"。同屏必须标"厂商自述" |
...
- [ ] P7（Context）与 P25（Codex 数字）回源核对后再上屏。
- [ ] P27–P29 的"五个自查维度"需要补证：现有素材只覆盖 Codex 与 Claude 系的接口形态，
      其余产品改成不带产品名的通用说法，或补证后再点名。
- [ ] 证据强度标注的**统一写法**（建议：一手 / ⚠️ 厂商自述 / ⚠️ 降级取证 / 趋势级）定稿后写进 `CONTEXT.md`。
