...
## 信息流与目录结构（2026-09-21 主干定稿升格）

**信息流三规则**：

1. **单向加工**：`research/` 证据档案 → `research/` 分篇判读 → `final/` 定稿；review 改变定稿 → 反向同步分篇。
2. **final/ 只收过筛结论**（判据见 [`final/03-practice-backbone.md`](final/03-practice-backbone.md) §0）；未采纳线索留 `research/` 原位（防重复检索），不删不隔离。
3. **过程件不入流**：仓库根 `.tmp-*` 不入库、不进任何一层，版本收口即清理。

```text
harness_governance/
├── README.md / CURRENT.md     # 入口件（地图 / 热区）
├── research/                  # 调研层：8 件（判读 ×2、证据档案 ×4、关系判定 ×1）+ figures/
└── final/                     # 定稿层：03 主干总纲 + 层 README（04 手册规划中）
```
...
| 文件 | 内容 | 状态 |
|---|---|---|
| [`final/03-practice-backbone.md`](final/03-practice-backbone.md) | ★ 实践主干总纲：宪法 3 / 工件 8 / 回路 7 / 节奏 3 / 组织与边界 7 + 筛选判据 + 六缺口诊断轴 | 宪法/工件/回路经用户确认（2026-09-21）；节奏/组织与边界待复核 |
| `final/04-practice-manual.md` | 手册级操作规程（SOP / 模板 / 判据表 / 统一 P0→P4 梯子 / Phase 0 三问表） | 规划中 |

## 核心论点（继承自已往轮次；操作化见 final/03）
...
2. **治理重心从"写对 spec"移到"工程化执行环境"**：linter / 结构测试 / 可观测性接入运行时 / gardener agent 定期扫漂移 / "每次犯错就工程化消灭该错误类别"——五段操作化即 `final/03`。
...
**指代消歧（2026-09-21 升格时定）**：档案正文遗留的"03 文档 / 03 文 / 03 号文档"指代不一——`research/01a`、`research/01b` 文中指 `research/01`（context 判读），`research/02b` 文中指 `research/02`（harness 判读）；**均与 `final/03` 无关**。新写引用一律用"层前缀 + 编号 + 节号"式（如 research/01 §2.2）。
...
- **SDD 批判与剩余后继形态光谱**（验证优先 / plan mode / 测试优先 + 历史判断层）→
  ...
  （本主题的"问题从哪来"与光谱定位记录在该处，冲突时**本目录 final/ 主张层为权威**）
...
promoted_at: 2026-09-21 # 主干定稿：research/ + final/ 信息流分层（final/03 总纲），用户定
scope: agent 执行链路治理：harness、context、规则文件纪律、门禁、漂移清理
layers:
  research/: 证据与判读（证据权威）
  final/: 定稿主张与操作规程（主张权威）
related:
