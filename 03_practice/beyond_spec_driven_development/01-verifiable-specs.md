...
critiques.md 的死结一是「自然语言 spec 无法自我验证」。验证优先派给出的不是"别写 spec"，而是**重新定义 spec 的合法性标准：一份 spec 只有在被某个验证器（测试运行器、command、类型检查器、模型检验器、对抗 reviewer）消费时才存在**。本篇对总览（本目录 README §1）做四向深挖：VSDD 讨论串的完整论点图谱（§1）、compilable specs 的真实实践样例（§2）、facts 的原文细节（§3）、失败面与团队落地（§4–5）。
...
![consistency tax 三角](figures/01-consistency-tax-triangle.svg)

> 三角模型图：两条红边是漂移路径（均有 2026 一手证据编号），绿边是唯一机器可校验的边；两个失效面与两种机制化回写直接标注。
