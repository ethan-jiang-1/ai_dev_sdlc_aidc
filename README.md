...
| 目录 | 规格 | 状态 | 说明 |
|---|---|---|---|
| `talk-ai-coding-evolution-agent-101/` | 19 页正文 + 2 停顿页 = 21 张 / 20–30 min；＋配套手册 20 页 | ★ **待 review** | **入门场**：面向**已经在动手搭 Agent、但不知道怎么把它搭稳**的人（**业务熟、AI 有初步理解、技术一般**）。**不是"零术语版"，是"术语按序入场"**——一条公式 `Agent = Model + Harness` + 五件事 + 四档。**文案 v6 已落盘**：上屏文案 21 张 ＋ 手册页版 20 页 ＋ 长版讲义；**机械结构全过，四轴复核已到第六轮（判"严重 0 条"）**，**待用户拍板后渲染 `05_output/`** |
