...
- **结构还原的加工**：原文正文小标题**不带编号**，文首「导读」有一份 1–6 的项目清单。
  转录时把该编号**加到了小标题上**（`## 01｜…` ~ `## 06｜结尾`）以便导航——**编号来自导读，不是原文标题自带**。
- **2026-09-22 复核（用户要求查图）**：
  - **正文**：抽 83 句回查原文页面，**逐句一致**（未发现改写或增删）。
  - **图片**：图1–图6 已按原文**全分辨率**重新下载——原先其中 3 张是缩水版
    （图1 1899×836→1080×475、图3 1137×516→1080×490、图6 1902×836→1080×475），
    现与原文图**逐字节一致**；非正文 4 张保留核验时版本。
  - **顺序**：按原文出现顺序重下全部 10 张（6 正文 + 4 非正文），与归档一一对应，无错位、无遗漏。
