...
- 本维判据**大部分可用 tier-0/1** 采证（grep 契约测试存在、CI 配置含漂移检查），是九维里自动化潜力最高的——checklist-A/B 落条时优先 tier-0 化。
...
- ~~仓库无显式 spec 时本维如何打分~~ **已裁决**（v1.7，[ADR 度量语义](../adr/2026-09-21-metric-semantics.md) D5）：spec 类判据为条件判据（schema `applicable` 字段），未采用 SDD 记 **N/A**——不扣分（A 子族既定口径）；契约钉子、声明可判定等无 spec 也适用的判据照常评。checklist 落条时逐条标 `applicable`。
