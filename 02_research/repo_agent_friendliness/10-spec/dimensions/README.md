# dimensions — 维度定义层（一维一文件）

九个维度（v1.3 起），每个一个 MD，序号连续 01–09。当前权威定义总表见 [`../framework.md`](../framework.md) §3.4；本目录是各维的展开定义，checklist-A/B 的判据按维归入。
...
每个维度文件的固定结构：核心问题 / 为什么独立成维 / 边界声明 / 判据族 / 依据 / 泛化注意 / 开放问题（含 B 面的维另设"形态 B 子族"小节，可选）。

**增维或改边界时**：先过目录 README 的正交纪律（宁拆不混、边界成文、同词异义拆开），再改本表与 `../framework.md` §3.4，两处必须同步。
