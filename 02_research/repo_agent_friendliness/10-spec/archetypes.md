...
> 状态：v1.3（2026-09-21）。定义层组件，权威链：[`framework.md`](framework.md) §3.5 → 本文件。
...
| 漂移 | **A/B 共同的敌人**（不定调为分型依据），守护对象不同 | A 守 spec/契约与确定性实现一致（测试/CI 可钉） | B 守行为定义与预期行为一致（behavioral drift，需 evals + 永久看护） |
...
- v1.3（2026-09-21）：**同步框架 v1.3**——八维措辞更新为九维（本文件评估面表述跟随）；B 线⑨升一等（定义见 [`dimensions/09-vcs-collaboration.md`](dimensions/09-vcs-collaboration.md) 与 [`line-b-agentic-repo.md`](line-b-agentic-repo.md)）；权威链引用改指 framework §3.5。六层映射表未动（2026-09-21 复核：⑨ 的协作面映射 L3 执行编排/L6 约束恢复的交叠部分，待试测后决定是否补入映射表）。
