...
7. **产出**：`report.md`（模板见 `30-runs/README.md`）——门禁判定 + 各维短板分 + 加权分（权重未定型则留空并注明）+ 按序整改清单；回本目录 `CURRENT.md` 登记一行。
...
- **checkpoint 隔离**：权重未校准前，报告只给分维短板分与门禁判定，**不给总分**（framework §4.4）。
...
| 组件 | 状态 |
|---|---|
| framework（九维、聚合结构、tier） | ✅ 已定型（v1.3） |
