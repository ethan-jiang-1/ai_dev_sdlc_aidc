...
7. **产出**：bundle 内 `report.md`（模板见 `20-instruments/bundle-format.md`）——门禁判定 + **A–D 总评评级**（framework §4.2 刻度，校准前可用）+ 各维短板分 + 加权数值分（权重未定型则留空）+ 按序整改清单（⑤❌ 置首 → 门禁 ⚠️ 次段 → 其余 ❌ 按维度短板分升序 → ⚠️ 随其维）。外部 pilot **不在本系统登记**——bundle 的 manifest 即记录；本仓库自举完成后按常规更新 `CURRENT.md` 状态。
