...
回顾两问（`04-harness-governance.md` §7.3/§7.8/§7.13-3）：①**Böckeler 之问**——"如果传感器从不触发，是高质量还是检测不足？我们需要类似代码覆盖率之于测试那样的方法来评估 harness 覆盖率和质量"；②**Ronacher 之塔**——团队对代码库的共享理解在无声腐烂，"没有任何 harness 指标会报告这一点"。
...
3. 因此本文对 04 号文档 §7.13-3 的修正案：把"harness 自身的覆盖率/质量如何度量——观测日无任何一方给出可操作答案"改为**分层表述**——"指令文件文本质量度量已有工具级雏形（agent-config-harness/schliff，2026H2）；传感器元覆盖与团队理解层腐烂度量在 2026-09-20 观测日仍为零方案"。后者才是真正无主的开放问题。
