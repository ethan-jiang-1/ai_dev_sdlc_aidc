...
3. 新增遗留问题：**harness 自身的覆盖率/质量如何度量**（Böckeler 之问，§7.3）——**分层表述（已按 [04b](04b-harness-academic-and-metrics.md) 三轮深挖收窄）**：指令文件文本质量度量已有工具级雏形（agent-config-harness / schliff 两源独立 + arXiv:2604.07236 方法论）；但**传感器元覆盖率**（哪些失效模式没有任何 sensor 在看）与**团队理解层腐烂度量**（Ronacher 之塔）在 2026-09-20 仍为零方案——后者才是真正无主的开放问题。
